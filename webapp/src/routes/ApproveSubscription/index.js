...
const ApproveSubscription = () => {
  ...
  const { t } = useTranslation('ApproveSubscriptionRoute')
  ...
  const handleOnSubmit = async () => {
    try {
      const response = await tokenSaleUtil.approveSubscription(user, payload)
    ...
    >
      <Card className={classes.card}>
        ...
        <CardContent className={classes.fields}>
          ...
          />
          <TextField
            label={t('max_allocation')}
            value={payload.max_allocation || ''}
            onChange={handleOnChange('text')('max_allocation')}
            variant="filled"
          />
        </CardContent>
        <CardActions>
...
ApproveSubscription.propTypes = {}

export default memo(ApproveSubscription)
